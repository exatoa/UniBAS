...
BEGIN
	...
	select
		...
		,(select top 1 thetext from longdescs where bug_id = b.id)	as [description]
		,creation_ts
		,delta_ts
	from bugs b
...
BEGIN
	...
	BEGIN
		insert into UniBAS.dbo.comment
		select 
			 @site_id
			,comment_id
			,bug_id
...
BEGIN
	...
	insert into UniBAS.dbo.attachment
	select 
			 @site_id
			,attach_id
			,bug_id
...
BEGIN
	...
	insert into UniBAS.dbo.attach_data
	select
		 @site_id
		,id
		,thedata
...
BEGIN
	...
	--Query문 생성
	insert into UniBAS.dbo.history
	select
		 @site_id
		,bug_id
		,who
...
BEGIN
	...
	insert into UniBAS.dbo.relationship
	select 
		 @site_id
		,blocked
		,dependson
	...
	SELECT @url = base_url FROM UniBAS.dbo.[site] WHERE id = @site_id
	...
	SELECT 
		...
		,cast(substring(value, len(@url)+1, 20)as integer)	as dest_bug_id
...
BEGIN
	...
	insert into UniBAS.dbo.reference
	select
		 @site_id
		,bug_id
		,value
...
BEGIN
	...
	insert into UniBAS.dbo.monitor
	select
		 @site_id
		,bug_id
		,who
...
BEGIN
	...
	insert into UniBAS.dbo.project
	select 
		 @site_id
		,id
		,null
...
BEGIN
	...
	insert into UniBAS.dbo.component
	select 
		 @site_id
		,id
		,product_id
...
BEGIN
	...
	INSERT INTO UniBAS.dbo.keyword
	SELECT 
		 @site_id
		,id
		,name
...
BEGIN
	...
	insert into UniBAS.dbo.bug_keyword
	select 
		 @site_id
		,bug_id
		,keywordid
...
BEGIN
	...
	insert into UniBAS.dbo.resolution
	select 
		 @site_id
		,id
		,sortkey
...
BEGIN
	...
	INSERT INTO UniBAS.dbo.severity
	SELECT 
		 @site_id
		,id
		,sortkey
...
BEGIN
	...
	insert into UniBAS.dbo.[priority]
	select 
		 @site_id
		,id
		,sortkey
...
BEGIN
	...
	insert into UniBAS.dbo.[platform]
	select 
		 @site_id
		,id
		,sortkey
...
BEGIN
	...
	insert into UniBAS.dbo.[version]
	select 
		 @site_id
		,product_id
		,id
...
BEGIN
	...
	insert into UniBAS.dbo.milestone
	select 
		 @site_id
		,product_id
		,id
